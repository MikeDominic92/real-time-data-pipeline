...
def main() -> None:
    ...
    parser.add_argument(
        "--initial-rate",
        type=int,
        help="Initial rate for increasing load test"
    )
    parser.add_argument(
        "--final-rate",
        type=int,
        help="Final rate for increasing load test"
    )
    parser.add_argument(
        "--step-size",
        type=int,
        help="Step size for increasing load test"
    )
    parser.add_argument(
        "--step-duration",
        type=int,
        help="Step duration for increasing load test"
    )
    parser.add_argument(
        "--burst-size",
        type=int,
        help="Messages per second during burst"
    )
    parser.add_argument(
        "--burst-duration",
        type=int,
    ...
    parser.add_argument(
        "--rest-duration",
        type=int,
        help="Duration of rest between bursts"
    )
    parser.add_argument(
        "--num-bursts",
        type=int,
    ...
    if args.test_type == "constant":
        ...
        results = [runner.run_constant_load_test(
        ...
        output_file = os.path.join(
            args.output_dir,
            f"constant_load_test_{timestamp}.json"
        )

    elif args.test_type == "increasing":
        if not (
            args.initial_rate and args.final_rate
            and args.step_size and args.step_duration
        ):
            parser.error(
        ...
        output_file = os.path.join(
            args.output_dir,
            f"increasing_load_test_{timestamp}.json"
        )

    else:  # burst
        if not (
            args.burst_size and args.burst_duration
            and args.rest_duration and args.num_bursts
        ):
            parser.error(
        ...
        output_file = os.path.join(
            args.output_dir,
            f"burst_load_test_{timestamp}.json"
        )
